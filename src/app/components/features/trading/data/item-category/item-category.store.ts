...
const CACHE_NAME = 'item-categories';
...
export const ItemCategoryStore = signalStore(
  ...
  withMethods(
    (
      store,
      service = inject(ItemCategoryService),
      ...
      const catalogCache = createCachedCrudLoader<ItemCategory>({
        ...
        loadCatalog: () => catalogCacheStore.loadCatalog<ItemCategory>(CACHE_NAME),
      ...
      const cacheCatalogEntry = (itemCategory: ItemCategory): void => {
        ...
        void catalogCacheStore.persistCatalog(CACHE_NAME, store.itemCategory().catalog);
      };
