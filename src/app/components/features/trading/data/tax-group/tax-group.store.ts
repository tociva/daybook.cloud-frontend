...
export const TaxGroupStore = signalStore(
  ...
  withMethods(
    (
      store,
      service = inject(TaxGroupService),
      ...
      const catalogCache = createCachedCrudLoader<TaxGroup>({
        ...
        loadCatalog: () => catalogCacheStore.loadCatalog<TaxGroup>(CACHE_NAME),
      ...
      const cacheCatalogEntry = (taxGroup: TaxGroup): void => {
        ...
        void catalogCacheStore.persistCatalog(CACHE_NAME, store.taxGroup().catalog);
      };
