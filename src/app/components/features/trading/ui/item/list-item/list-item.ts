...
export class ListItem implements OnInit, OnDestroy {
  ...
  handleOnFilesSelected(files: File[]): void {
    const [file] = files;
    this.store.dispatch(itemActions.uploadBulkItems({ file }));
  }
  ...
  private loadItems(): void {
    this.route.queryParams.pipe(
      ...
      const search = params.search?.length ? params.search : [{query: '', fields: ['description', 'name', 'code', 'displayname', 'barcode']}];
