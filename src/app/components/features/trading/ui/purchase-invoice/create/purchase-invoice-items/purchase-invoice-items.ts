...
export class PurchaseInvoiceItems implements OnInit {
  
  // required signal input from parent
  readonly form = input.required<FormGroup<PurchaseItemsDetailsForm>>();
  ...
  private readonly onTaxOptionChangeEffect = effect(() => {
    ...
    for(const itemRow of this.itemsArray.controls) {
      ...
      for(let idx = 0; idx < this.taxColumnCount; idx++) {
        taxFormArray.push(this.purchaseInvoiceFormService.buildPurchaseItemTaxForm());
      }
  ...
  private refreshItemRow = (itemRow: FormGroup<PurchaseItemForm>) => {
    const currency = this.currency();
    const minorunit = currency?.minorunit ?? TWO;
  ...
  private updateItemRowTaxControls = (itemRow: FormGroup<PurchaseItemForm>, taxGroupId?: string) => {
    if(taxGroupId) {
      const currency = this.currency();
      const minorunit = currency?.minorunit ?? TWO;
  ...
  addItemRow() {
    const newGroup = this.purchaseInvoiceFormService.buildPurchaseItemForm(this.taxColumnCount);
