...
export type PurchaseInvoiceBulkUploadValidationOptions = Readonly<{
  minorUnit?: number;
}>;
...
export function validatePurchaseInvoiceBulkUploadPayload(
  payload: BulkUploadPayload,
  options: PurchaseInvoiceBulkUploadValidationOptions = {},
): readonly string[] {
  ...
  const seenInvoiceKeys = new Map<string, number>();
  ...
  for (const [index, invoice] of invoices.entries()) {
    ...
    if (!number) {
      pushError(
        invoiceError(index, undefined, 'number is required and must be a non-empty string.'),
      );
    }
    ...
    if (!vendorName) {
      pushError(
        invoiceError(
          index,
          number ?? undefined,
          'vendorname is required and must be a non-empty string.',
        ),
      );
    }

    if (number && vendorName) {
      const invoiceKey = `${vendorName.trim().toLowerCase()}::${number.trim().toLowerCase()}`;
      const previousIndex = seenInvoiceKeys.get(invoiceKey);
      if (previousIndex !== undefined) {
        pushError(
          invoiceError(
            index,
            number,
            `duplicate invoice number for vendor "${vendorName}" (also used by invoice ${previousIndex + 1}).`,
          ),
        );
      } else {
        seenInvoiceKeys.set(invoiceKey, index);
      }
    }
    ...
    if ('vendoraddress' in invoice && invoice['vendoraddress'] !== undefined) {
      validateAddress(invoice['vendoraddress'], 'vendoraddress', index, number, pushError);
    }
    ...
    for (const [itemIndex, item] of items.entries()) {
      ...
      const itemItemTotal = validateRequiredNonNegativeNumber(
        item,
        'itemtotal',
        index,
        number,
      ...
      const itemSubtotal = validateRequiredNonNegativeNumber(
        item,
        'subtotal',
        index,
        number,
      ...
      const itemGrandTotal = validateRequiredNonNegativeNumber(
        item,
        'grandtotal',
        index,
        number,
      ...
      const discamount = validateOptionalNonNegativeNumber(
        item,
        'discamount',
        index,
        number,
      ...
      const taxamount = validateOptionalNonNegativeNumber(
        item,
        'taxamount',
        index,
        number,
      ...
      if (taxes !== undefined && taxes !== null) {
        if (!Array.isArray(taxes)) {
          pushError(
            itemError(index, number, itemIndex, itemName ?? undefined, 'taxes must be an array.'),
          );
    ...
    if (itemtotal !== null && !amountsEqual(itemtotal, sumItemTotal, minorUnit)) {
      pushError(
        invoiceError(
          index,
          number ?? undefined,
          'itemtotal must equal the sum of item itemtotal values.',
        ),
      );
    }
    ...
    if (subtotal !== null && !amountsEqual(subtotal, sumSubtotal, minorUnit)) {
      pushError(
        invoiceError(
          index,
          number ?? undefined,
          'subtotal must equal the sum of item subtotal values.',
        ),
      );
    }
      ...
      if (taxValue !== null && !amountsEqual(taxValue, sumTaxAmount, minorUnit)) {
        pushError(
          invoiceError(
            index,
            number ?? undefined,
            'tax must equal the sum of item taxamount values.',
          ),
        );
      }
    }
    ...
    if (grandtotal !== null && !amountsEqual(grandtotal, sumGrandTotal + roundoff, minorUnit)) {
      pushError(
        invoiceError(
          index,
          number ?? undefined,
          'grandtotal must equal the sum of item grandtotal values plus roundoff.',
        ),
      );
    }
  }
...
function validateAddress(
  ...
  if (!isRecord(value)) {
    pushError(
      invoiceError(invoiceIndex, invoiceNumber ?? undefined, `${fieldName} must be an object.`),
    );
    return;
  }
...
function validateTax(
  ...
  if (!isRecord(value)) {
    pushError(
      taxError(invoiceIndex, invoiceNumber, itemIndex, itemName, taxIndex, 'must be an object.'),
    );
    return;
  }
