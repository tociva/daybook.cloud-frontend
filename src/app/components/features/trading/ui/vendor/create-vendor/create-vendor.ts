...
import { Vendor, vendorActions, VendorCU, VendorStore } from '../../../store/vendor';

@Component({
  selector: 'app-create-vendor',
  imports: [TwoColumnFormComponent, SkeltonLoader, ItemNotFound],
  templateUrl: './create-vendor.html',
  styleUrl: './create-vendor.css'
})
export class CreateVendor implements OnInit {
  ...
  readonly formFields = signal<FormField[]>([
    ...
    { key: 'pan', label: 'PAN', type: 'text', required: false, group: 'Basic Details'},
  ...
  readonly title = signal('Vendor Setup');
  ...
  private loadErrorEffect = effect(() => {
    const error = this.vendorStore.error();
  ...
  ngOnInit(): void {
    ...
    if (lastSegment === 'create') {
      this.mode = 'create';
      this.successAction.set(vendorActions.createVendorSuccess);
      this.failureAction.set(vendorActions.createVendorFailure);
      this.loading = false;
    } else if (lastSegment === 'edit') {
      this.itemId.set(this.route.snapshot.paramMap.get('id') || null);
      if(this.itemId()) {
        this.successAction.set(vendorActions.updateVendorSuccess);
        this.mode = 'edit';
        this.loading = true;
        this.store.dispatch(vendorActions.loadVendorById({ id: this.itemId()!, query: { filter: JSON.stringify({
          "include": [
            { "relation": "country" },
            { "relation": "currency" }
          ]
  ...
  handleSubmit = (data: Vendor) => {
    ...
    if(this.mode === 'create') {
      ...
      this.store.dispatch(vendorActions.updateVendor({ id: this.itemId()!, vendor }));
    }
  }
}
